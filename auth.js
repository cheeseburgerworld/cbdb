...
async function initAuth() {
  ...
  if (session) {
    ...
    // Could not verify session right now (network/offline/tab-wake). Do NOT log
    // the user out. Keep whatever the optimistic cache painted and try again
    // on the next load. The session is still in the library's own store.
    // (state stays as restoreCache() left it.)
  } else {
    // init() succeeded and positively returned no session → genuinely logged out.
